...
module ZKPVerification_BLS6_6 where
...
import qualified Plutus.Script.Utils.V2.Typed.Scripts.Validators as V2V

import Params_BLS6_6
...
data TypedGroth
instance V2V.ValidatorTypes TypedGroth where
  type instance DatumType TypedGroth    = VerifyDatum
  type instance RedeemerType TypedGroth = ()

typedGrothValidator :: V2V.TypedValidator TypedGroth
typedGrothValidator = V2V.mkTypedValidator @TypedGroth
  $$(PlutusTx.compile [|| mkVerificationValidator ||])
  $$(PlutusTx.compile [|| wrap ||])
  where
    wrap = V2V.mkUntypedValidator @VerifyDatum @()

grothValidator :: PlutusV2.Validator
grothValidator = V2V.validatorScript typedGrothValidator
